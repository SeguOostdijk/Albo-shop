...
const isValidPostal = (v: string) =>
	/^([A-Za-z]\d{4}([A-Za-z]{3})?|\d{4})$/i.test(v.trim());
...
export default function CheckoutPage() {
	...
	const [transferInfo, setTransferInfo] = useState<{ orderId: number } | null>(
		null,
	);
	...
	const getUnitPrice = (product: { price: number; memberPrice?: number }) =>
		memberValidated && product.memberPrice
			? product.memberPrice
			: product.price;
	...
	const subtotal = useMemo(
		() =>
			items.reduce(
				(acc, item) => acc + getUnitPrice(item.product) * item.quantity,
				0,
			),
	...
	const handleContinueShipping = () => {
		...
		if (!isValidPostal(postalCode)) {
			toast.error(
				"El código postal debe tener formato válido (ej: 1000 o B7600)",
			);
			return;
		}
		markDone(2, 3);
	...
	const handleSubmit = async () => {
		...
		if (paymentMethod === "transfer" && !user && otpStep !== "verified") {
			...
			try {
				...
				toast.error(
					"Error al enviar el código. Verificá tu email e intentá de nuevo.",
				);
				setOtpStep("idle");
			}
			return;
	...
	const createOrder = async () => {
		...
		try {
			...
			if (!data.success) {
				toast.error(
					data.error || "Error al procesar el pedido. Intenta nuevamente.",
				);
				return;
			}
			...
			<div className="container mx-auto px-4 py-12">
				<div className="max-w-2xl mx-auto">
					...
					<p className="text-muted-foreground mb-6">
						Tu pedido fue generado correctamente. Realizá la transferencia con
						los datos que figuran abajo.
					</p>
					<div className="rounded-lg border p-6 space-y-2 mb-6">
						<p>
							<strong>Número de pedido:</strong> {transferInfo.orderId}
						</p>
						<p>
							<strong>Banco:</strong> Banco de la Nación Argentina
						</p>
						<p>
							<strong>CBU:</strong> 0110464040046411693330
						</p>
						<p>
							<strong>Alias:</strong> ALBO.SHOP
						</p>
						<p>
							<strong>Nombre:</strong> Club Atlético Independiente de San
							Cayetano
						</p>
						<p>
							<strong>CUIT:</strong> 30-70708050-3
						</p>
					</div>
					<button
						type="button"
						onClick={() =>
							navigator.clipboard
								.writeText("0110464040046411693330")
								.then(() => toast.success("CBU copiado"))
					...
					<p className="text-base font-medium text-center">
						...
						<strong>alboshopcai@gmail.com</strong> indicando tu número de
						pedido.
					</p>
					<p className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-md px-4 py-3 text-center mt-4">
						Si no se envía el comprobante de pago dentro de las 48 horas, el
	...
	return (
		<div className="container mx-auto px-4 py-6">
			<Breadcrumbs
				items={[{ label: "Carrito", href: "/cart" }, { label: "Checkout" }]}
			/>
				...
				<div className="lg:col-span-2 space-y-4">
						...
						summary={
							doneSteps.has(2) ? (
								<div className="text-sm text-muted-foreground space-y-0.5">
									...
									<p>
										{address}, {city}, {PROVINCES[province]}, CP {postalCode}
									</p>
					...
					>
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										setFirstName(
											e.target.value.replace(/[^a-zA-ZÀ-ÿñÑ\s]/g, ""),
										)
									}
									required
								/>
						...
						<div className="mt-6 pt-5 border-t border-border">
							...
							>
								<div
									className={`flex items-center justify-between p-4 border rounded-lg ${shippingMethod === "pickup" ? "border-primary bg-primary/5" : "border-border"}`}
								>
								...
								<div
									className={`flex items-center justify-between p-4 border rounded-lg ${shippingMethod === "standard" ? "border-primary bg-primary/5" : "border-border"}`}
								>
									<div className="flex items-center gap-3">
									...
									<span className="font-medium">
										{subtotal >= 75000 ? "Gratis" : formatCurrency(1)}
									</span>
								</div>
							</RadioGroup>
						</div>
						...
						summary={
							doneSteps.has(3) ? (
								<div className="text-sm text-muted-foreground space-y-0.5">
									...
									{memberValidated && (
										<p className="text-green-600">✓ Precio socio aplicado</p>
									)}
								</div>
							) : null
						}
					>
						...
						<div className="rounded-lg border border-border p-4 space-y-3 mb-6">
							<Label className="text-sm font-medium">
								Ingresa tu Nombre y Apellido de socio
							</Label>
						...
						{paymentMethod === "mercadopago" && (
							<>
								<p className="mt-4 text-sm text-muted-foreground">
									Al presionar Pagar serás redirigido a MercadoPago para
						...
						{paymentMethod === "transfer" && (
							<>
								<p className="mt-4 text-sm text-muted-foreground">
									Al presionar Pagar se mostrarán los datos de la cuenta
									bancaria y las instrucciones para completar la transferencia.
								</p>
								...
								{user && (
									<button
										...
										disabled={
											isLoading || !doneSteps.has(1) || !doneSteps.has(2)
										}
										onClick={handleSubmit}
									>
										{isLoading
...
function StepCard({
	...
	return (
		<div
			className={`rounded-xl border bg-card transition-all duration-200 ${isLocked ? "opacity-50" : ""}`}
		>
			{/* Header */}
