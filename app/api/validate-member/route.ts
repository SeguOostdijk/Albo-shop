...
export async function POST(req: Request) {
	try {
		...
		const { data: inactiveData, error } = await supabase
			.from("members")
			.select("id, member_name, is_active, last_payment_at")
		...
		if (!inactiveData.is_active) {
			return NextResponse.json({
				...
				valid: false,
				message:
					"miembro inactivo. Presione el link de abajo para renovar su suscripción",
			});
		}
		...
		if (!isMembershipCurrent(data.last_payment_at)) {
			return NextResponse.json({
				ok: true,
				valid: false,
				message:
